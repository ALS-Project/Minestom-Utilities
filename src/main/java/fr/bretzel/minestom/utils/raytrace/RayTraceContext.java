...
public record RayTraceContext(Instance instance, Vec start, Vec end, Vec direction, BlockMode blockMode, FluidMode fluidMode) {
    ...
    public RayTraceContext(Player player, double distance, BlockMode blockMode) {
        this(player.getInstance(), player.getPosition().withY(y -> y + player.getEyeHeight()).asVec(), player.getPosition().withY(y -> y + player.getEyeHeight()).asVec().normalize().mul(distance), player.getPosition().direction(), blockMode, FluidMode.NONE);
    }
    public RayTraceContext(Player player, double distance, BlockMode blockMode, FluidMode fluidMode) {
